...
function dbInfo(dbPath) {
    ...
    var sq = 'Select c.chat_identifier, h.id, m.text, m.date, a.filename from message m , ' +
        'chat c inner join chat_message_join cm on m.rowid = cm.message_id and c.rowid = cm.chat_id ' +
        ' inner join chat_handle_join chj on chj.chat_id = c.rowid inner join handle h on h.rowid = chj.handle_id ' +
        ' left outer join message_attachment_join ma on m.rowid = ma.message_id left outer join attachment a on ma.attachment_id = a.rowid ' +
        ' where a.filename is not null ' +
        'order by chat_identifier, m.date';

    var ss = "SELECT text, date FROM message";

    var rVal = [];
    db.each(sq, function( err, row ) {
        row.datef = new Date(row.date*1000 + 978307200000).toLocaleString();
        //console.log( row );
        rVal.push(row);
    });
    return rVal;
